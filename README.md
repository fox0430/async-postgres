...
- **Retry classification** — `isTransientError(err)` tells a retry loop whether retrying with the same config may succeed: true for failures known to clear (a lost or refused connection, a timeout, a transient SQLSTATE such as `40001`), false for anything unclassified (`PgSecurityError`, `PgProtocolError`, `PgConfigError`, `PgStateError`, cancellation). True does not mean replaying is safe — a connection lost during `COMMIT` may have committed — nor that the connection is gone, so check `conn.state` before discarding it. Cap retries: a mistyped host name or a server that stays down is transient too.
